# -*- coding: utf-8 -*-
"""
Task 2: Four-poet style analysis
================================
What it does:
  1. Scores every poem of the four poets (config.POETS), giving poem-level
     emotional distributions (paper Supplementary Table S1 + Figure 2 boxplot);
  2. Six pairwise Welch t-tests with Holm correction (the numbers in the
     table note);
  3. Signature-imagery list (paper Figure 3) and high-frequency imagery list
     (Supplementary Appendix C), both under the double-evidence filter plus
     the non-poetic function-word screen (config.EXCLUDE).

Outputs (output/task02_poet_style/):
  - tableS1_poet_distributions.csv, tableS1_pairwise_tests.csv
  - signature_imageries.csv, appendixC_highfreq_imageries.csv
  - figure2_boxplots_EN/CN.png, figure3_signature_imageries_EN/CN.png,
    volcano_plots_EN/CN.png (not used in the final paper)

Run: python task02_poet_style_analysis.py   (about 1-2 minutes)
"""
import math
import numpy as np
import pandas as pd
...
def main():
    ...
    out = utils.task_outdir('task02_poet_style')
    ...
    # ---------- 1. Poem-level scoring and distributions ----------
    print('=== 1. Poem-level emotional distributions (Table S1) ===')
    score_arr = {}
    rows = []
    for n in config.POETS:
        ...
        print(f'{n}: n={len(a)}  mean={a.mean():+.4f}  median={np.median(a):+.4f}  sd={a.std(ddof=1):.4f}')
    pd.DataFrame(rows, columns=['Poet', 'Poems', 'Mean', 'Median', 'SD']
                 ).to_csv(f'{out}/tableS1_poet_distributions.csv', index=False, encoding='utf-8-sig')

    # ---------- 2. Pairwise tests ----------
    print('\n=== 2. Welch pairwise tests (Holm correction) ===')
    df_test = utils.welch_all_pairs(score_arr)
    print(df_test.round(4).to_string(index=False))
    df_test.to_csv(f'{out}/tableS1_pairwise_tests.csv', index=False, encoding='utf-8-sig')

    # ---------- 3. Signature and high-frequency imagery lists ----------
    print('\n=== 3. List construction (double-evidence filter + poeticity screen) ===')
    ...
    hf = {n: utils.highfreq_list(n, poet_img_cnt, char_val, bi_val)
          for n in config.POETS}
    for n in config.POETS:
        print(f'[{n}] signature:', ', '.join(x[0] for x in sig[n]))
        print(f'[{n}] high-freq:', ', '.join(x[0] for x in hf[n]))

    # saved as comparison tables (one column per poet)
    pd.DataFrame({n: [f'{x[0]}({x[1]}, {x[2]:.1f}, {x[3]:+.2f})' for x in sig[n]]
                  for n in config.POETS}
                 ).to_csv(f'{out}/signature_imageries.csv', index=False, encoding='utf-8-sig')
    pd.DataFrame({n: [f'{x[0]}({x[1]}, {x[2]:+.2f})' for x in hf[n]]
                  for n in config.POETS}
                 ).to_csv(f'{out}/appendixC_highfreq_imageries.csv', index=False, encoding='utf-8-sig')
    ...
    for lang in ['en', 'cn']:
        ...
        plt.tight_layout()
        fig.savefig(f'{out}/figure2_boxplots_{lang.upper()}.png', dpi=300)
        plt.close(fig)

    # ---------- Figure 3: signature-imagery bars (12 per poet) ----------
    for lang in ['en', 'cn']:
        ...
        fig.savefig(f'{out}/figure3_signature_imageries_{lang.upper()}.png', dpi=300)
        plt.close(fig)

    # ---------- Volcano plots (not used in the final paper) ----------
    for lang in ['en', 'cn']:
        ...
        for ax, n in zip(axes.flat, config.POETS):
            ...
            # label the poet's 12 signature imageries; the EN version labels English
            # glosses only and uses adjustText to avoid overlap; without adjustText
            # labels are placed directly
            try:
        ...
        plt.tight_layout(rect=[0, 0, 1, 0.98])
        fig.savefig(f'{out}/volcano_plots_{lang.upper()}.png', dpi=300)
        plt.close(fig)
